...
6. Build zones, sequence them spatially, and evaluate
...
## Step 6 — Sequence zones and tracts

Once zones are built, we sequence them spatially: first the **visit order
between zones** (which zone to visit first, second, etc.), then the **visit
order of tracts within each zone**.

```{r sequence}
# Zone visit order via flood-fill + TSP on min pairwise tract distances
# (access_points for haversine fill-in are pulled from the plan)
plan_partition <- surveyzones_sequence_zones(plan_partition, distances)
plan_no_partition <- surveyzones_sequence_zones(plan_no_partition, distances)

# Tract visit order within each zone
...
hull is monochromatic. Numbered labels show the **zone visit order** from
SPIN_NH seriation; dashed white lines connect consecutive zones.
...
make_map <- function(plan) {
  ...
  # Color by group_id when zone_sequence is available, otherwise by partition_id
  if (!is.null(plan$zone_sequence) && "group_id" %in% names(plan$zone_sequence)) {
    group_lookup <- plan$zone_sequence |> select(zone_id, group_id)
    pts <- pts |>
      left_join(group_lookup, by = "zone_id") |>
      mutate(color = color_by(group_id))
  } else {
    pts <- pts |> mutate(color = color_by(partition_id))
  }
  ...
  m <- mapboxgl(bounds = pts, style = mapbox_style("satellite-streets")) |>
    add_fill_layer(
      id                 = "hulls",
      source             = hulls,
  ...
  # Add zone sequence lines and labels if available
  if (!is.null(plan$zone_sequence)) {
    # Build zone center points with sequence order
    centers <- plan$zones |>
      left_join(plan$zone_sequence, by = c("zone_id", "partition_id")) |>
      left_join(
        access_pts |> select(tract_id, geometry),
        by = c("center_tract_id" = "tract_id")
      ) |>
      st_as_sf() |>
      st_transform(5880L) |>
      mutate(label = as.character(zone_order))

    # Build lines connecting consecutive zones within each partition
    seq_lines <- centers |>
      arrange(partition_id, zone_order) |>
...
surveyzones_export_plan(plan_partition, path = "output/rj_zones")
```

### Plan comparison

`surveyzones_compare()` gives a quick summary across plans:
